...
description: 一条命令把你在「一堂」的全部作业评分拉到本地：只读调用 whyai CLI，生成 JSON 数据真相源 + 自包含 HTML 工作台（评分分布 / 搜索 / 档位筛选 / 小屏卡片，点击任意作业可查看完整详情：题目、逐题作答与老师点评），可选归档作业正文全文。带内嵌环境核对（版本基线 + 登录态）与保密模式边界（零出境，不调 chat、不上传、不写服务端）。当用户要求「更新作业评分」「拉取作业学分」「刷新作业清单」「查我作业得了几分」或要生成作业评分报告时触发。
triggers:
  - 更新作业评分
  - 拉取作业学分
  - 刷新作业清单
...
| 技能目录/`workbench/` | **工作台前端**（评分分布、关键词搜索、3/4/5/6 分与未评分筛选、小屏卡片化、点击查看作业完整详情）+ 双击启动入口 |
...
- 本技能**只调用只读接口**：`yitang homework list`、`yitang homework show`、`me info`、学习报告等 `risk: read` 能力。
...
- 所有成绩数据只落本地，不回传。
- 工作台本地服务含一个**只读详情代理** `GET /__api/homework/<id>`：id 必须为纯数字，仅代理上述只读命令 `homework show`，无任何写路径；服务只监听 `127.0.0.1`。
...
**查看作业完整详情**：点击任意一行 / 卡片打开详情弹窗。经本地服务打开时，会自动调只读接口 `yitang homework show` 拉取该作业的**完整内容**——列表接口只返回第 1 题作答、不含题目，详情接口才是全量——并按「题目标题 / 题目说明 / 我的答题 / 老师点评（含总评）」逐题分层展示，附「官方页面」直达链接（`yitang.top/homework/<id>`）与一键复制；结果在服务运行期间缓存，重开秒出。拉取失败（如 CLI 未登录、双击 `file://` 打开被浏览器拦截）时自动回退显示正文归档（`raw/`，仅第 1 题作答），也可用「导入正文归档…」手动加载。
