...
解锁授权的是「**已知边界内**的出境」，不是「未知边界上的出境」。解锁的代价是**不再逐次确认** —— 所以每次出境前仍要能说清「发什么、发给谁」；说不清就**先停下问用户**。
...
- **清单可能随时变**：若 `whyai models list --json` 里找不到本文所用的默认 alias，说明清单已更新 —— **改用清单内可用的同类模型并告知用户**，不要照旧名字硬试。
...
> 本节是**会话锚定与中断恢复**的正文：锚定时机、断联后的续用、以及「id 丢了怎么办」。

### 会话锚定：什么时候必须把 id 写下来

**本地不留历史** —— YAI 侧历史全在服务端，`-c <conversation-id>` 是接上它的**唯一入口**。而会话 id 若只活在当前上下文里，**上下文压缩、更换 Agent、重开窗口都会让它消失**；一旦消失，最直觉的补救「开新会话 + 把旧历史贴回去」**正是唯一会真正造成巨量输入的路径**（回灌内容会成为新会话的输入基数，且**随每一轮重发** —— 这是少数会让成本成倍上升的动作）。
...
id 丢了，唯一的危险动作是**开新会话 + 把旧历史手动回灌进 prompt**——只有这条会真正造成巨量输入（代价随轮数累积，当前视为最危险的路径）：
...
> ⚠️ 这是传输层行为，**不是新的一轮**。诊断「配额异常」时不要误判成重复扣费（已据源码证伪）。
...
9. **判据随版本失效**：本文的放行 / 禁止清单以特定 CLI 版本实测为准。**命令行为与本文不符时，先停下向用户说明，不要照旧执行。**
...
- **命令行为与本文不符时**（CLI 版本可能已变）：先停下，把「哪条不符、影响哪些操作」说清楚再继续；不要带着不确定做出境类调用。
